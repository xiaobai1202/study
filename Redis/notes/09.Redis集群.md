...
主从和哨兵可以解决高可用，高并发问题。但是依然有两个问题没有解决： 海量数据存储问题、高并发写问题

使用分片集群可以解决上述问题，分片集群特征：

1. 集群中有多个master
2. 每个master都可以有多个slave
3. master之间通过ping监测彼此健康状态
4. 客户端请求可以访问集群任意节点，最终都会被转发到正确节点

redis 5.0 以后可以直接通过 redis-cli cluster 创建集群

    对于redis cluster的插槽，数据不是与节点绑定，而是与插槽绑定。redis会根据key的有效部分计算插槽值，分为两种情况：
    1. key中包含“{}”且“{}”中包含至少一个字符，“{}”中的部分为有效部分
    2. key中不包含“{}”，整个key都是有效部分
    
    场景: 控制某一类数据在一个节点， 可以使用{}保证有效部分一样，就会全部放到一个节点上

加入一个新的节点到集群： redis-cli cluster add-node 添加节点  然后  redis-cli cluster reshared 分配插槽

自动故障转移流程

1. 首先是该实例与其他实例失去连接
2. 然后是疑似宕机
3. 最后是确定下线，自动提升一个slave作为新的master


手动故障转移/更新节点流程

利用cluster failover 命令可以手动让集群中某个master 宕机，切换到执行cluster failover命令的这个节点，实现无感知的数据迁移。流程如下：

![redis-cluster手动故障转移流程](./img/Redis-Cluster-Failover.png)

手动的failover有三种模式：

1. 缺省： 就是默认的流程，上图中的1-6步骤
2. force： 省略了offset一致性校验
3. takeover： 直接执行第五步，忽略数据一致性，忽略master状态和其他master意见
